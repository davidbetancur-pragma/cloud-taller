...
| Estructural | **Adapter** | `app/src/app/shared/patterns/structural/post.adapter.ts` | El origen de contenido (`posts.data.ts`) usa la forma típica de una API REST (`{ userId, id, title, body }`). El resto de la app trabaja con el modelo `Post` (`authorId`, `summary`). El adapter es el único lugar que conoce esa forma. |
